...
/**
 * Get the tool names needed for memory management (read, write, edit).
 * Only returns names NOT already in the provided set.
 */
...
/**
 * Get only the read tool name for read-only memory access.
 * Only returns names NOT already in the provided set.
 */
...
export function getToolNamesForType(type: string): string[] {
  ...
  return toolNames.filter((n) => BUILTIN_TOOL_NAMES.includes(n));
}
